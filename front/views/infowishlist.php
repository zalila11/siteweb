...
require_once ("../../config.php");
			   ...
			   function afficherproduit($nom)
			{
				$sql='SELECT nomproduit as nom
				FROM `wishlist` WHERE nomproduit=nomproduit';
				$connexion=config::getConnexion();
				$rep=$connexion->query($sql);
				$rep->bindValue(':nom',$nom);
			    $nom=$rep->fetch(PDO::FETCH_OBJ);
			    return $nom;
			   }
	if (isset($_GET['nom'])) {
			    
			    $name=afficherproduit($_GET['nom']);
			}
?>
<?php
require_once ("../core/wishlistC.php");
$wishlist=new wishlistC();
$listeWishlists=$wishlist->afficherwishlist();
$db=config::getConnexion();
$nb1=$db->query('SELECT COUNT(*) as nb1 
				FROM `wishlist` WHERE nomproduit=nomproduit')->fetchColumn();


?>
<h3><td>Vous Avez <?php echo $nb1; ?> Produits Dans Votre Wishlist Qui Sont:</h3>
	<?php $listeWishlists=$wishlist->afficherwishlist();
          foreach($listeWishlists as $row){?>
            <td><h3><?php echo $row['nomproduit'];}?></h3></td> <br>			
