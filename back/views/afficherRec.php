...
<!--heder end here-->
<ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="index.html">Home</a><i class="fa fa-angle-right"></i>Gestion SAV</li>
            </ol>
...
<h2>Liste des Reclamations</h2>
	<?PHP
include "../core/afficherReclamation.php";
$reclamation=new reclamationa();
$listeReclamations=$reclamation->afficherReclamation();
?>
<table border="3">
<tr>

<th>Nom Reclamation</th>
<th>date Reclamation</th>
<th>message Reclamation</th>
...
foreach($listeReclamations as $row){
	...
	<td><?PHP echo $row['nomRec']; ?></td>
	<td><?PHP echo $row['dateRec']; ?></td>
	<td><?PHP echo $row['messageRec']; ?></td>
	<td><?PHP echo $row['sujetRec']; ?></td>
	<td><?PHP echo $row['email']; ?></td>
		<td><?PHP echo $row['etat']; ?></td>

	<td><form method="POST" action="supprimerReclamation.php">
	<input type="submit" name="supprimer" value="supprimer">
	<input type="hidden" value="<?PHP echo $row['nomRec']; ?>" name="nomRec">
	</form>
	</td>
	<td><a href="modifierReclamation.php?nomRec=<?PHP echo $row['nomRec']; ?>">
	<input type="button" name="modifier" value="modifier"></a></td>
</tr>
<?PHP
	}

	require_once '../core/modifierReclamation.php';
	$rect= new reclamationm();
	$i=$rect->nombreReclamation();
            echo "<center><p class=\"est\" style='color: green'>Nombre De Reclamation non traitée : $i</center>";
...
<?PHP
	...
	require_once '../core/modifierReclamation.php';
	$rect= new reclamationm();
	$n=$rect->nombreReclamationt();
            echo "<center><p class=\"est\" style='color: green'>Nombre De Reclamation traitée : $n</center>";
...
<h2>Rechercher une Reclamation</h2>
	<?PHP
include "../core/rechercherReclamation.php";
$reclamation=new reclamationre();
//echo ($_POST["rech"]);
if (isset($_POST["rech"]))
{
$n=$_POST["rech"];
$listeReclamations=$reclamation->rechercherReclamation($n);
}
