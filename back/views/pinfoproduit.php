...
$db=config::getConnexion();
$nb1=$db->query('SELECT COUNT(*) FROM `produit` WHERE idproduit=idproduit')->fetchColumn();
$nom=$db->query('SELECT nomproduit FROM `produit` WHERE idproduit=idproduit')->fetchColumn();
?>
	<tr>
		<td><h3>Vous Avez: <?php echo $nb1; ?> Produits Dans Votre Stock
			Qui Sont :</h3><?php
			
			 $listeProduits=$produit->afficherproduit();
          foreach($listeProduits as $row){?>
            <td><h3><?php echo /*$nom*/$row['nomproduit'];}?></h3></td>
